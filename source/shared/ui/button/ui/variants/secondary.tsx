...
export const Secondary = ({ children, loading, disabled, className, ...props }: primaryProps) => {
  ...
  return (
    <button
      type="submit"
      className={`${buttonCls} w-full ${className} h-11`}
      disabled={disabled}
      onFocus={() => setIsFocused(true)}
      onBlur={() => setIsFocused(false)}
      {...props}
    >
      {!loading ? children : <Loader height={44} width={44} variant={BUTTON_VARIANTS.SECONDARY} />}
    </button>
  );
};
