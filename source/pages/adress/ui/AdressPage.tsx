  ...
  return (
    <>
      <div>
        ...
        <div className="font-medium text-[12px]">Добавьте Ваши адреса и удобные способы доставки для быстрого оформления
          покупок.
        </div>
      </div>
      {
        !address[0] ?
          <div className="mt-8 w-[251px]">
            <Link className="uppercase px-[50px] font-semibold py-[14px] text-[12px] bg-black text-white"
          ...
          :
          <div className="mt-5">
            {address.map(a => <ItemAddress key={a.id} address={a} />)}
      ...
      <ShowModalAddress func={() => console.log("da")} text={"Удалить адрес?"} />
    </>
  );
};
