...
const Sidebar = () => {
  ...
  return (
    <>
      <ShowModalMb text="выйти из учетной записи?" func={() => {
        console.log("Выход");
        changeVisibleExit(false);
      }} />
      <div className="w-[20%] flex flex-col	gap-[16px]  pl-[40px] h-full max-[1050px]:hidden">
        <Link
          className={classNames("w-full uppercase  flex justify-start  text-[16px] font-semibold", !url[2] && url[1] === "lk" ? "text-black" : "text-black-200")}
          href="/lk">
          Личный кабинет
        </Link>
        {links.map(l => <ItemLinkLk key={l.href} link={l} active={url[2] === l.href} />)}
        <button onClick={() => changeVisibleExit(true)}
      ...
      <div className={s.mobile}>
        <Select color={"#F0F0F0"} options={options} value={value ? value : "Личный кабинет"}
                onChange={
                  // @ts-ignore
