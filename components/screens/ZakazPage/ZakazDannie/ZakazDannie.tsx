...
const ZakazDannie = ({title, main}:IProps) => {
    ...
    let defValMain = {...Object.fromEntries(aaaarrrr.main.map(n => [ n.type, n.val ])), ...Object.fromEntries(aaaarrrr.adres.map(n => [ n.type, n.val ]))
    }
    const {
        register,
        handleSubmit,
    ...
    return (
        <div className={s.root}>
            ...
            <form className={s.inputs} onSubmit={handleSubmit(onSubmit)}>
                {main?aaaarrrr.main.map(p => {
                    // @ts-ignore
                    return <InputForm error={errors[p.type] && errors[p.type].message} register={register} type={p.type}
                                      reset={resetField}
                    form={true}/>
